...
import { t as imported_t_progressbar } from '../translation_logic.js';

const CSS_PATH = 'css/components/progress_bar_component.css';
let css_loaded_for_progressbar = false;

async function load_pb_styles_if_needed() { // Omdöpt till load_pb_styles_if_needed för att vara mer specifik
    if (!css_loaded_for_progressbar && typeof load_css === 'function') {
        if (!document.querySelector(`link[href="${CSS_PATH}"]`)) {
            try {
                await load_css(CSS_PATH);
                css_loaded_for_progressbar = true;
            } catch (error) {
                console.warn("ProgressBarComponent: Failed to load CSS:", error);
            }
        } else {
            css_loaded_for_progressbar = true; // Already in DOM
        }
    } else if (!css_loaded_for_progressbar && typeof load_css !== 'function') {
        console.warn("ProgressBarComponent: load_css (importerad) not available.");
    }
}

export async function create_progress_bar_component(current_value, max_value, options = {}) {
    await load_pb_styles_if_needed();

    if (typeof create_element !== 'function' || typeof imported_t_progressbar !== 'function') {
        console.error("ProgressBarComponent: create_element or t (importerad) not available!");
    ...
    const progress_element_attributes = {
        ...
        'aria-label': options.label || imported_t_progressbar('progress_bar_label', { defaultValue: 'Progress' })
    };
    if (options.id) {
...
console.log("[ProgressBarComponent.js] ES6 Module loaded.");